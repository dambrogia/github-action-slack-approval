...
const is_stream_1 = __importDefault(__nccwpck_require__(3752));
...
module.exports = __nccwpck_require__(1403);
...
const slack_1 = __importStar(__nccwpck_require__(7501));
async function run() {
    const errorAction = core.getInput('errorAction') === slack_1.Action.APPROVE
        ? slack_1.Action.APPROVE
        : slack_1.Action.REJECT;
    function setOutput(approved, rejected, error) {
    ...
    try {
        ...
        const slackClient = new slack_1.default(slackToken, {
            appId: slackAppId,
            channel: slackChannel,
            historyLimit: Number(slackHistoryLimit),
            pollPause: Number(slackPollPause),
            pollTimeout: Number(slackPollTimeout),
            errorAction: errorAction === slack_1.Action.APPROVE ? slack_1.Action.APPROVE : slack_1.Action.REJECT,
            timeoutAction: timeoutAction === slack_1.Action.APPROVE ? slack_1.Action.APPROVE : slack_1.Action.REJECT
        });
        const approvalText = `${approvalPrefix}${runId}`;
        const rejectionText = `${rejectionPrefix}${runId}`;
        const approvalRequestMessage = slack_1.default.getApprovalRequestMessage(repository, runId, approvalText, rejectionText);
        ...
        if (error instanceof Error && errorAction === slack_1.Action.REJECT) {
            core.setFailed(error.message);
        }
        setOutput(errorAction === slack_1.Action.APPROVE, errorAction === slack_1.Action.REJECT, true);
    }
}
run();
...
class Slack {
     ...
     */
    static getApprovalRequestMessage(repository, runId, approvalText, rejectionText) {
        return [
            '```',
    ...
    async getChannelId(cursor) {
        ...
        if (conversations.response_metadata &&
            conversations.response_metadata?.next_cursor) {
            return await this.getChannelId(conversations.response_metadata?.next_cursor);
        }
        return undefined;
    }
    async getRecentMessages() {
        ...
        if (!response.ok) {
            throw Error(`Invalid reponse from slack when reading messages: ${response?.error || 'unknown'}`);
        }
    ...
    async *poll() {
        ...
        const secondsPassed = () => Math.floor(Date.now()) - started;
        let i = 0;
    ...
    async approve(approvalNeedle, rejectionNeedle) {
        try {
